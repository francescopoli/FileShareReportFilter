...
<#       
    .DESCRIPTION
        ...
        for instance:
        Folders : path longer than 256 chars
        Files: path longer than 256 chars or size greater than 2Gb
...
FilterReport function
    ...
    FileShare: only FileShare with: Files or Folder where path is longer than 256 chars or file bigger than 2GB (any of this condition
    File: only files that have path longer than 256 chars or file is bigger than 2gb
    Folders: only folders that have path longer than 256 chars
    ...
    param(
        ...
        switch ($type)
        {
            "FileShareInventory"{
                ...
                {
                    $error = [int]$entry.File_IsCrossed256CharsCount + [int]$entry.FileCountGreaterThanThresholdMB + [int]$entry.Folder_IsCrossed256CharsCount
                    if ( ($error) -gt 0) 
                    {
                        $errors++
                        $result += $entry
            ...
            "FolderInventory"{
                ...
                {
                    $done++
                    if ( ($entry.IsCrossed256Chars -eq "TRUE") ) 
                    {
                        $count++
                        $errors++
            ...
            "FileInventory"{
                ...
                {
                    $done++
                    if ( ($entry.IsCrossed256Chars -eq "TRUE") -or  ($entry.'Greater than 2GB' -eq "TRUE")) 
                    {
                        $count++
                        $errors++
